...
def load_process(data):
    ...
    melted_data = pd.melt(data, id_vars=['Country Name'], value_vars=value_vars, var_name='year', value_name='Export')
    ...
    nan_count_export = sorted_data['Export'].isnull().sum()
    nan_counts_by_year = sorted_data.groupby('year')['Export'].apply(lambda x: x.isnull().sum())
    sorted_data = sorted_data[sorted_data['continent'] != 'not found']
    return sorted_data

def continent_Export(data):
    export_by_continent_year = data.groupby(['continent', 'year'])['Export'].sum().reset_index()
    return export_by_continent_year
...
data1 = continent_Export(sorted_data)
